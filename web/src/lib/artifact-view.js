 ...
 * ── 为什么需要向导，而不是一句"直接说就行" ──────────────────────────────
 *
 * 作品是**模型产出的**，没有"新建"按钮。这带来两个问题，而且它们出现在不同阶段：
 *
 *   1. 一开始：用户打开空列表，本能是找那个按钮，找不到就得出"功能还没做好"的结论；
 *   2. 之后：他做出一两份之后，指引（原本只画在空状态里）就再也不出现了 ——
 *      于是"原来还能画流程图、还能写 Vue 组件"这件事永远没人知道。
 *
 * 所以指引不能只当空状态的插图，得是一个**随时打开得了的入口**。
 * 向导做的事很朴素：让人选一种类型、看见它擅长什么、写一句想要什么，
 * 然后**把话拼好送进输入框**。它不创建任何东西 —— 创建的仍然是模型。
 *
 * `lead` / `tail` 是拼话术用的：把用户那句朴素的描述，补成模型一看就知道
 * 该产出什么形态的说法。写得像人话，不像填表。
 */
export const ARTIFACT_RECIPES = [
  {
    kind: 'web',
    blurb: '能点、能算、能交互的网页。小工具、计算器、看板都算。',
    lead: '帮我做一个网页：',
    tail: '。样式简洁一点，直接能用。',
    examples: [
      '贷款计算器，输入本金、年利率、期数，实时算出月供和总利息',
      '一个团队周报看板，按人分组、可以折叠，数据先用假数据填上',
    ],
  },
  {
    kind: 'vue',
    blurb: 'Vue 3 单文件组件项目，可以拆成多个组件和工具函数。',
    lead: '用 Vue 3 写一个组件：',
    tail: '。拆成 App.vue 和 components/ 下的子组件。',
    examples: [
      '可复用的数据表格，支持排序、分页和空状态',
      '一个多步骤表单，带上一步/下一步和校验提示',
    ],
  },
  {
    kind: 'markdown',
    blurb: '给人读的文档：方案、报告、说明书。支持表格，也能插流程图。',
    lead: '整理成一份文档：',
    tail: '。结构清晰一点，该用表格的地方用表格。',
    examples: [
      '把刚才聊的内容整理成方案文档，要有背景、目标、方案对比和落地步骤',
      '一份新人上手指南，覆盖环境准备、常见问题和联系人',
    ],
  },
  {
    kind: 'mermaid',
    blurb: '流程图、时序图、甘特图、状态机 —— 用文字描述，渲染成图。',
    lead: '画一张图：',
    tail: '。',
    examples: [
      '用户下单到发货的流程，包含支付失败和缺货两个分支',
      '登录鉴权的时序图：浏览器、网关、认证服务三方',
    ],
  },
  {
    kind: 'svg',
    blurb: '手画的矢量图：图标、示意图、海报。放大不糊。',
    lead: '画一张矢量图：',
    tail: '。',
    examples: [
      '一套四个的状态图标（成功、失败、进行中、已取消），线性风格',
      '一张系统架构示意图，三层，带箭头和标注',
    ],
  },
  {
    kind: 'code',
    blurb: '成段的代码或配置：脚本、SQL、YAML。只看源码，不预览。',
    lead: '写一段代码：',
    tail: '。',
    examples: [
      '一个 Python 脚本，把一个目录下的 csv 合并成一个 xlsx',
      '一份 docker-compose，跑 postgres + redis + 一个 node 服务',
    ],
  },
]

export function findRecipe(kind) {
  return ARTIFACT_RECIPES.find((item) => item.kind === kind) || null
}

/**
 * 把「类型 + 一句描述」拼成给模型的话。
 *
 * 抽成纯函数是为了能被测试直接盯住：这句话拼歪了（比如把类型说丢了），
 * 表现是"我选了画图，它给我写了段代码"，而用户完全看不出是哪一步错的。
 */
export function composeArtifactPrompt(kind, description) {
  const text = String(description || '').trim()
  if (!text) return ''
  const recipe = findRecipe(kind)
  if (!recipe) return text
  // 用户自己已经把类型说清楚了就不再套模板，否则会拼出"帮我做一个网页：帮我做一个网页…"
  if (text.startsWith(recipe.lead)) return text
  return `${recipe.lead}${text}${recipe.tail || ''}`
}

...
助手写一个 200 行的 HTML 报表，贴在对话气泡里的代价有三层：一屏全是代码、
**改一行要整段重发**、用户拿不到成品。所以「作品」单独存（`src/artifacts/`）：
对话里只留一张卡片，正文进面板预览、切版本、下载。

界面上有**两个入口，分工不同**：对话右边的抽屉只看**当前这条会话**的产出（边聊边看），
侧栏的「作品」进的是**跨会话的作品库**（独立页面，可搜索、按类型筛、跳回产出它的那条对话）。
库的空状态就是**创建指引** —— 作品是模型产出的、没有"新建"按钮，
而用户对着空列表的本能是找那个按钮，找不到就会得出"这功能没做好"的结论。

**一份作品是一组文件，不是一段文本。** 只存一段正文等于要求模型把样式、脚本、
组件全塞进一个文件里 —— 而"能拆模块"恰恰是它值得被存下来的原因。落盘形状：
...
<dataDir>/users/<username>/artifacts/
  ...
  <id>/v1/app.js
  <id>/v2/…          第 2 版（自包含快照，不是相对上一版的差量）
```

两个取舍：**每版是完整快照**（清理旧版就只是 `rm -rf v1/`，不必算引用；
算错的后果是某一版永远打不开），**模型改动时只发变了的文件**（`write` 是合并语义，
`update` 是单文件内的定点替换 —— 这才是多文件真正省 token 的地方）。

支持的类型与各自的渲染路径：

| kind | 内容 | 怎么预览 |
|---|---|---|
| `web` | 多文件静态网页 | 把作品内的 `<link>` / `<script src>` / `<img src>` **内联**成一份文档 |
| `vue` | Vue 3 SFC 项目 | 前端用 `@vue/compiler-sfc` 现编译，沙箱内用 blob URL 组模块图 |
| `markdown` | 文档，含 ```mermaid 围栏 | 转义优先的渲染器 + mermaid 渲成 SVG |
| `mermaid` | 单张图 | 同上 |
| `svg` / `code` | 矢量图 / 代码 | 直接显示 / 只看源码 |

Vue 运行时与 mermaid 都**自托管并按需加载**（主包只 +12KB，其余进懒加载分块）：
预览沙箱默认不能出网，CDN 那条路根本不通。所以给模型的提示词里写死了
「除 `vue` 外不能 import 任何第三方包」—— 不说的话它会照常写 CDN 引用，然后白屏，
而它看不到这个结果。

**渲染是这个功能的安全边界，不是它的展示细节。** 作品内容是模型生成的，而模型的输入里
有用户的邮件和网页 —— 谁能往里塞脚本，取决于谁能给这个用户发东西。两道互相独立的防线：
...
配套的不变量在服务端：`/v1/artifacts/:id/raw` 一律 `text/plain` + `nosniff`，
**本服务从不以 HTML 的身份吐出任何模型生成的内容** —— 因此不存在"同源的、
内容由模型决定的 URL"这种东西。同样的理由，盘上那些 `.html` 决定了

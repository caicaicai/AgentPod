 ...
 * ── 单副本 ──────────────────────────────────────────────────────────────
 *
 * **CRON_ENABLED 只应在一个副本上打开。** 任务表在共享盘上，占坑（claimSlot）
 * 只在进程内串行 —— 两个副本同时 tick，同一个任务会被点两次。真要多副本，
 * 需要的是一把跨进程的租约（qm 用 Postgres advisory lock），那属于换存储后端的事。
 * 这条写进了 config 校验的提示里，启动日志也会说明当前是不是调度副本。
 *
 * ── 失败不退格 ──────────────────────────────────────────────────────────
 *

...
| `COMPACTION_ENABLED` | `1` | 上下文压缩：会话快撑满上下文窗口时把更早的对话折成摘要。⚠️ **这件事一直在发生**（pi 的默认值就是开的），新增的是让它可见、可调，见[上下文压缩](#上下文压缩) |
...
| 单价 | 输入 / 输出 / 缓存读入三档，单位统一是**每百万 token** —— 上游价目表怎么报的就怎么填，不要换算成每 token。三项全空 = 这条模型**不定价**，见[用量计价](#用量计价) |

#### 用量计价

三个单价一填，管理台用量页就长出金额那一列：按人、按模型、按天，外加一个部署总计。
币种是部署级的（`USAGE_CURRENCY`，默认 `USD`），**只是个符号** —— 不做汇率换算，也不支持
一个部署里多种币。

看这些数字之前，有三件事必须知道：

- **没有价格历史。** 折算永远用**此刻**库里那个价，包括三个月前的那些行。改了价，历史数字
  跟着变。它回答的是「按今天的价，这些用量值多少钱」，不是「当时收了多少钱」。
- **未定价 ≠ 0。** 没填价的模型算出来是 `null`，界面上写「未定价」，绝不会被当 0 加进合计。
  混着未定价模型的行会标 `≥` 并点名缺的是哪几条 —— 一个静悄悄偏小的账单是最没人会质疑的那种错。
  注意**填 0 是一个真实的答案**（"这个模型免费"），留空才是"我们不知道"。
- **压缩的 token 不在账上。** 上下文压缩会**另外**调一次模型写摘要，那次调用不走会话事件流，
  所以进不了 `ap_usage`。也就是说这里算出的成本**比上游真实账单偏低**，差的就是压缩那部分。

和台账一样：这是**可归因的成本估算，不是账单**。对外收钱以模型网关那份为准。

**用户分组**在「分组」页维护，一个人一个组，决定他**能用哪些模型**和**能用多少 token** —— 它不是角色
（能不能管别人是 `role`），也不是隔离边界（会话、作品、记忆一直按账号隔离）。标了「默认」的那个组，
新建的账号会自动进去。没有分组的人能用的是那些**不限可用范围**的模型，且不受额度限制。
删掉一个分组时，组里的人退回「无分组」，模型的可用范围里也会把它摘掉 —— 账号和模型本身都不删。

#### Token 额度

每个分组可以配两个额度，**都是 0 / 留空 = 不限**：

| 字段 | 说明 |
|------|------|
| 总额度 | **累计**用量的上限，永不重置。烧完就停，要继续用得管理员调高 |
| 每日额度 | 每天的上限，跨零点归零。"零点"按 `QUOTA_TIMEZONE`（默认 `Asia/Shanghai`）算 |

几件要先知道的事：

- **按人算，不是全组共用一个池子。** 配 100 万 = 组里每个人各有 100 万。共用池的话，
  一个人跑几个大任务就能让整组停摆，而被停的人既看不出是谁烧的、也做不了任何事。
- **口径是 input + output，不含缓存读入** —— 与管理台「用量」页那张表同一个口径。
- **它是一道事前闸门**：用量要这一轮跑完才知道，所以判据是"截至上一轮为止用了多少"。
  最后那一轮**会超一点**（开始时还剩 1 个 token 也照样让它跑完），超出的部分记在账上、下一轮被拦。
  想一个 token 都不超，就得在流式输出中途掐断模型 —— 那点量买不到这个体验。
- **失败的 run 不扣额度**（失败的 run 本来就不记账），所以重试不会白扣。
- 拦下来时：总额度用完回 **403**（等下去没用），当日额度用完回 **429** 并带 `retryAfterMs`
  （过了零点自己就好）。定时任务据此决定"退避重试"还是"别再打了"。
- 改额度**下一轮对话就生效**，不需要谁重新登录。
- ⚠️ 它**不是安全边界**：把一个人移出分组就解开了限制。它防的是"跑飞的任务把预算烧光"。
...
会话快撑满模型的上下文窗口时，更早的对话会被折成一段摘要，好让对话继续下去。
**这不是新加的能力** —— pi 一直默认在做。缺的是"能看见、能调"，而缺席的代价是两个用户
一定会注意到、却没有任何地方解释的现象：

1. 长会话里某一轮忽然十几秒不出字（压缩要**另外调一次模型**写摘要）；
2. 模型对前面聊过的细节记不清了（切点之前的对话只剩一段摘要）。

两个都真实，两个都没解释 —— 于是它们一起被归因成"这个模型变笨了"。

现在：压缩过程会发 `compaction` 事件帧，历史里在切点留一条分隔线
（**历史本身是完整的** —— pi 的会话是 append-only，变的只是**模型**看到的那份），
用户也可以自己挑个时候手动压一次。

| 变量 | 默认 | 说明 |
|------|------|------|
| `COMPACTION_ENABLED` | `1` | 自动压缩。⚠️ 关掉的代价是长会话**直接撞上下文上限报错**，而不是慢一点 |
| `COMPACTION_RESERVE_TOKENS` | `0` | 给摘要本身留多少 token 预算。`0` = 用 pi 的默认值（16384）。调小 = 摘要更短更糙，丢的正是"模型还记得什么" |
| `COMPACTION_KEEP_RECENT_TOKENS` | `0` | 保留多少最近的对话不折。`0` = 用 pi 的默认值（20000）。调大更安全，但留得越多能折的越少，压缩就来得更频繁 —— 而每次都是一次额外的模型调用 |

`POST /v1/compact` 手动压一次（输入框里打 `/压缩`）。它值得有，是因为自动压缩**总是发生在
用户正等回答的时候**；挑一个自己不着急的时刻先压掉，下一轮就不会被那十几秒打断。
它走**和一轮对话同样的额度与并发闸门** —— 它要调模型，占的是同一类资源。

⚠️ **压缩自己烧的 token 进不了 `ap_usage`** —— pi 写摘要走的是 `completeSimple`，
不经过会话事件流。这正是管理台用量页偏低的原因，见[用量计价](#用量计价)。
...
| `QUOTA_TIMEZONE` | `Asia/Shanghai` | 分组的**每日** token 额度在哪个时区跨零点归零。额度本身配在分组上（见上面「Token 额度」），这里只管日界 |
| `USAGE_CURRENCY` | `USD` | 管理台用量页上金额的币种符号。**纯展示** —— 不做汇率换算，一个部署一种币。单价配在每条模型上（见[用量计价](#用量计价)） |
...
| GET | `/v1/admin/groups` | 分组清单，每个组带 `userCount`、`modelCount` 与两个额度；`ungrouped` 是没有分组的人数，`quotaTimezone` 是每日额度几点归零 |
| POST | `/v1/admin/groups` | 建分组。`{ name, description, isDefault, tokenQuota, dailyTokenQuota }`，最多一个默认分组（设了新的旧的自动取消）。两个额度 0 / 留空 = 不限 |
| PATCH | `/v1/admin/groups/:id` | 改名 / 改说明 / 设为默认 / 改额度 |
...
模型配了单价之后，这三个接口还会每行带一个 `cost`，外加一个 `pricing` 块
（`{ currency, cost, partial, unpricedModels }`）。一条模型都没定价时，这个块就是
`{ enabled: false }`，界面据此一列金额都不画 —— 画一列全是"—"只会让人以为是数据没加载出来。
这些数字能说明什么、不能说明什么，见[用量计价](#用量计价)。
...
| POST | `/v1/compact` | 手动压缩一条会话的上下文：`{ sessionKey, instructions? }`。普通 POST 而不是 SSE —— 中间没有任何可流式的东西（模型在写摘要，一个字都不吐），只有一个结果；但它**要调模型，可能十几秒**。走和一轮对话同样的额度与并发闸门。路径是 `/v1/compact` 而不是 `/v1/sessions/:key/compact`：那个前缀把剩下的整段都当 sessionKey 解析（同一个理由让会话搜索去了 `/v1/search`） |
| GET | `/v1/models` | 可用模型列表 |

**SSE 事件类型：** `run_start` · `model` · `thinking` · `text` · `text_end` · `tool_call` · `tool_result` · `retry` · `compaction` · `usage` · `final` · `error`

`compaction` 帧带 `{ state: 'start'|'end', reason: 'manual'|'threshold'|'overflow', … }`。
摘要正文**从不下发** —— 客户端要知道的是"发生了压缩、上下文有多大"，
不是去读一份自己对话的复述。
...
└── telemetry/          进程内指标 + 落库的按人 token 用量台账 + 分组额度闸门
